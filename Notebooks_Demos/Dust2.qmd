...
def analyze_b_site_anchoring(dem, round_numbers=None):
    tick_rate = 128  # Adjust based on demo
    ...
    for round_num in target_rounds:
        ...
        # Fix: Use "place" instead of "player_last_place_name"
        ...
        # Check for weapon fire/shots
        if hasattr(dem, 'shots') and not dem.shots.is_empty():
            round_shots = dem.shots.filter(
                (pl.col("tick").is_between(time_1_50, time_1_20)) &
                (pl.col("round_num") == round_num)
            )
            if not round_shots.is_empty():
                print(f"\nWeapon fire during timeframe (Round {round_num}):")
                shots_summary = round_shots.group_by("player_name").agg([
                    pl.col("weapon").mode().first().alias("primary_weapon"),
                    pl.col("tick").count().alias("shots_fired")
                ])
                print(shots_summary)
```
...
```{python}
# Focus on rounds where bomb was planted/exploded at B-site
b_site_rounds = dem.rounds.filter(pl.col("bomb_site") == "bombsite_b")["round_num"].to_list()
print(f"B-site rounds: {b_site_rounds}")

# Run analysis on B-site rounds specifically
analyze_b_site_anchoring(dem, round_numbers=b_site_rounds)

```

Debug
```{python}
# First, let's see what data is available in your demo
print("Available demo attributes:")
for attr in dir(dem):
    if not attr.startswith('_') and hasattr(dem, attr):
        try:
            obj = getattr(dem, attr)
            if hasattr(obj, 'shape'):
                print(f"{attr}: {obj.shape}")
